...
foreach (['helpers', 'config', 'series', 'roster', 'split', 'phases', 'analyze', 'group', 'bracket'] as $f) {
...
function bracket_report_matches() {
  ...
  $pst      = $report['match_parts_series_tag'] ?? []; // mid -> report's own series tag
  // lrg_metadata lazy-loads on access and its offsetExists is false until then,
  // so "?? []" would short-circuit before the cluster→region map is ever read.
  $clusters = [];
  try { $clusters = $meta['clusters']; } catch (\Throwable $e) {}
  ...
  foreach ($mpt as $mid => $sides) {
    ...
    $matches[] = [
      ...
      'series_tag'  => (string)($pst[$mid] ?? ''), // group games by the report's series, not a time bucket
      'series_num'  => 0,
      'string'      => '',
...
function bracket_generate() {
  ...
  if (count($matches) < 2) return ['generated_at' => time(), 'events' => []];
  ...
  $desc = [
    ...
    'name'     => $report['league_name'] ?? strtoupper($report['league_tag'] ?? 'Tournament'),
  ...
  $result = ['generated_at' => time(), 'events' => []];
...
// Per-report bracket config, read from report.bracket_config (or
// report.settings.bracket). Lets a report force the bracket shape when detection
// gets it wrong: overrides / stages (format) / divisions / months. See
// templates/bracket_config.example.json.
function bracket_config() {
  global $report;
  $c = $report['bracket_config'] ?? $report['settings']['bracket'] ?? null;
  if (!is_array($c)) return [];

  $config = [];
  if (!empty($c['overrides']) && is_array($c['overrides'])) {
    foreach ($c['overrides'] as $k => $v) $config['overrides'][(string)$k] = (int)$v;
  }
...
function bracket_json($result) {
  ...
  foreach ($result['events'] as $ev) {
    ...
    $events[] = ['name' => $ev['name'], 'stages' => $stages, 'placements' => $placements];
  }

  return ['generated_at' => $result['generated_at'], 'events' => $events];
}
...
function bracket_esc($v) { return htmlspecialchars((string)$v, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'); }
...
// Series popup title: "Team 1 vs Team 2 — Event — Stage Round" ($ctx is the
// already-localized "Event — Stage [Round]" tail; empty for context-less popups).
...
function bracket_name($name) {
  if (!preg_match('/^(bracket_[a-z_]+)(?: ([0-9A-Z]+))?$/', (string)$name, $m)) return $name;
  $s = locale_string($m[1]);
  if (!isset($m[2])) return $s;
  return strpos($s, '%n%') !== false ? str_replace('%n%', $m[2], $s) : trim($s.' '.$m[2]);
}

function bracket_render() {
  global $bracket_popups;
  if (!bracket_available()) return "";
  $result = bracket_generate();
  if (empty($result['events'])) return "";

  $bracket_popups = [];
  $multi = count($result['events']) > 1;

  $out = "<div class=\"content-text alert info bracket-notice\">".locale_string("bracket_notice")."</div>";
  foreach ($result['events'] as $ev) $out .= bracket_render_event($ev, $multi);
...
function bracket_render_event($ev, $multi) {
  ...
  $out = "";
  if ($multi) {
    // Sub-event titles need only the distinguishing suffix — the report name is
    // already the page title.
    $title = $ev['name'];
    $base  = $report['league_name'] ?? '';
    if ($base !== '' && str_starts_with($title, $base.', ')) $title = substr($title, strlen($base.', '));
  ...
  foreach ($ev['stages'] as $st) {
    if ($st['type'] === 'group_stage') {
      ...
      foreach ($st['series'] ?? [] as $s) $out .= bracket_match_card($s, "$event — $stage");
      $out .= "</div>";
    }
  }
  return $out;
...
function bracket_popup_id($mids, $title) {
  ...
  if ($gets) {
    $body .= "<a class=\"bracket-pop-link\" href=\"?league=$leaguetag&mod=matches-cards&gets=$gets".(empty($linkvars) ? "" : "&$linkvars")."\">".locale_string("bracket_view_matches")."</a>";
  }
  $id = 'bracket-pop-'.count($bracket_popups);
  $bracket_popups[$id] = ['title' => $title, 'body' => $body];
  return $id;
}
...
function bracket_render_grid($g, $ctx = '') {
  ...
  foreach ($ids as $row_id) {
    ...
    foreach ($ids as $col_id) {
      if ($row_id === $col_id) { $out .= "<td class=\"transparent\"></td>"; continue; }
      $cell = $g['grid'][$row_id][$col_id] ?? null;
      if (!$cell) { $out .= "<td>-</td>"; continue; }
...
function bracket_form_cell($r, $sep = false) {
  $cls = $sep ? 'separator' : '';
  if (!$r || ($r['w'] + $r['l'] + $r['d']) === 0) return "<td class=\"$cls\">-</td>";
  $rec = $r['w']."-".$r['d']."-".$r['l'];
  $tot = $r['w'] + $r['l'] + $r['d'];
...
function bracket_render_playoff($st, $event = '') {
  ...
  if (($st['phase_type'] ?? '') === 'elimination_round') {
    ...
    foreach (array_merge($b['ub_rounds'] ?? [], $b['lb_rounds'] ?? []) as $r) {
      foreach ($r['series'] as $s) $out .= bracket_match_card($s, "$base — ".bracket_name($r['name'] ?? ''));
    }
    $out .= "</div>";
  ...
  if (!empty($b['unplaced'])) {
    ...
    foreach ($b['unplaced'] as $s) $out .= bracket_match_card($s, "$base — $unp");
    $out .= "</div>";
  }
  return $out;
}
...
function bracket_edge_svg($card_pos, $row_h, $drop_entry = []) {
  ...
  foreach ($items as $it) {
    ...
    foreach ($items as $cand) {
      if ($cand['s']['start'] <= $it['s']['start'] || !in_array($w, $cand['s']['teams'])) continue;
      if ($next === null || $cand['s']['start'] < $next['s']['start']) $next = $cand;
    }
...
function bracket_render_section($rounds, $ctx = '') {
  ...
  foreach ($rounds as $ri => $round) {
    ...
    if ($ri + 1 < count($rounds)) $hdr .= "<div class=\"bracket-conn-sp\" style=\"width:".BRACKET_CONN_W."px\"></div>";
  }
  $hdr .= "</div>";
  ...
  foreach ($rounds as $ri => $round) {
    ...
    foreach ($round['series'] as $si => $s) {
      $card_pos[$s['key']] = ['x' => $ri * (BRACKET_COL_W + BRACKET_CONN_W), 'y' => (int)round(($si + 0.5) * $slot_size * BRACKET_SLOT_H), 's' => $s];
    }
  }
...
function bracket_de_columns($ub_rounds, $lb_rounds, $ub_to_lb) {
  ...
  for ($ui = 0; $ui < $n_ub; $ui++) {
    ...
    while ($lb_cursor < $gap_until) { $cols[] = ['ub' => null, 'lb' => $lb_rounds[$lb_cursor]]; $lb_cursor++; }
    $lb_slot = null;
    if ($align_lb !== null && $lb_cursor === (int)$align_lb && $lb_cursor < $n_lb) {
      $is_last_feeder = true;
      for ($uj = $ui + 1; $uj < $n_ub; $uj++) {
        $aj = $ub_to_lb[$uj] ?? $ub_to_lb[(string)$uj] ?? null;
        if ($aj !== null && (int)$aj === $lb_cursor) { $is_last_feeder = false; break; }
      }
      if ($is_last_feeder) { $lb_slot = $lb_rounds[$lb_cursor]; $lb_cursor++; }
    }
    $cols[] = ['ub' => $ub_rounds[$ui], 'lb' => $lb_slot];
  }
  while ($lb_cursor < $n_lb) { $cols[] = ['ub' => null, 'lb' => $lb_rounds[$lb_cursor]]; $lb_cursor++; }
  return $cols;
}
...
function bracket_render_de($ub_rounds, $lb_rounds, $ub_to_lb, $gf_series, $ctx = '') {
  ...
  if ($gf_series) {
    ...
    foreach ($gf_series as $s) $gf_html .= bracket_match_card($s, trim("$ctx — ".locale_string("bracket_gf"), " —"));
    $gf_html .= "</div>";
  }
  ...
  for ($i = 0; $i < $n_ub; $i++) {
    ...
    if ($v !== null) { if ((int)$v < $prev_lb_idx) { $is_monotone = false; break; } $prev_lb_idx = (int)$v; }
  }
  if (!isset($ub_to_lb[0]) && !isset($ub_to_lb['0']) && $n_ub > 0 && $n_lb > 0) $is_monotone = false;

  if ($coverage < 0.6 || !$is_monotone) {
    $out = "<div class=\"bracket-tree\"><div class=\"bracket-aligned\">";
    if ($ub_rounds) $out .= "<div class=\"bracket-label\">".locale_string("bracket_ub")."</div>".bracket_render_section($ub_rounds, trim("$ctx — ".locale_string("bracket_ub"), " —"));
    if ($lb_rounds) $out .= "<div class=\"bracket-label bracket-lb-label\">".locale_string("bracket_lb")."</div>".bracket_render_section($lb_rounds, trim("$ctx — ".locale_string("bracket_lb"), " —"));
  ...
  if ($ub_rounds) {
    ...
    for ($p = 0; $p < $first_ub_pos; $p++) {
      $out .= "<div class=\"bracket-col bracket-spacer\" style=\"height:".$ub_total_h."px\"></div><div class=\"bracket-conn-sp\" style=\"width:".BRACKET_CONN_W."px;height:".$ub_total_h."px\"></div>";
    }
    foreach ($ub_positions as $k => $pos) {
  ...
  if ($lb_rounds) {
    ...
    foreach ($columns as $ci => $col) {
      ...
      if ($ci + 1 < $n_cols) $out .= "<div class=\"bracket-hdr-conn\"></div>";
    }
    $out .= "</div>";
    ...
    foreach ($columns as $ci => $col) {
      ...
      if ($ci + 1 < $n_cols) $out .= "<div class=\"bracket-conn-sp\" style=\"width:".BRACKET_CONN_W."px;height:".$lb_total_h."px\"></div>";
    }
    $out .= "</div>";
  }
  ...
  if ($gf_html) {
    if ($ub_rounds && $lb_rounds) {
      ...
      $t0  = $gf0['teams'][0] ?? 0;
      $t1  = $gf0['teams'][1] ?? 0;
      // Each finalist's feeder stub + half the vertical riser lives in its own
      // group, so hovering a finalist lights up its path. The shared final stub
      // is drawn last in its own group, highlighted via CSS when either side is.
  ...
  foreach ($groups as $grp) {
    ...
    foreach ($grp as $t) {
      $out .= "<tr class=\"bracket-place$cls\" data-bracket-team=\"$t\">"
        ...
        ."<td class=\"bracket-place-team\">".team_logo($t)." <span class=\"bracket-place-team-name\">".bracket_label($t)."</span></td></tr>";
    }
    $place += $n;
  }
  return $out."</tbody></table>";
