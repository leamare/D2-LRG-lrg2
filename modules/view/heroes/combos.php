...
function rg_view_generate_heroes_combos() {
  ...
  if(isset($report['hero_triplets']) && !empty($report['hero_triplets'])) {
    ...
    if (check_module($parent_module."trios")) {
      $res['trios'] =  "<div class=\"content-text\">".locale_string("desc_heroes_trios", [ "liml"=>ceil($report['settings']['limiter_combograph']*0.25)+1 ] )."</div>";
  ...
  if(isset($report['hero_lane_combos'])) {
    ...
    if (check_module($parent_module."lane_combos")) {
      $res['lane_combos'] =  "<div class=\"content-text\">".locale_string("desc_heroes_lane_combos", [ "liml"=>round($report['settings']['limiter_combograph']*0.5)+1 ] )."</div>";
