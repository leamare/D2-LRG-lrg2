...
function rg_query_heroes_mvp(&$conn, $cluster = null, $players = null, $teamid = null) {
  ...
  $wheres = [];
  if (!empty($teamid)) {
    // Filter to matches played by $teamid, only counting the heroes played on that team's side
  ...
  if ($conn->multi_query($sql) === TRUE) echo "[S] Requested data for FANTASY HEROES MVP.\n";
