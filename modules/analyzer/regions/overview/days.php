...
foreach($result["regions_data"][$region]['overview']["days"] as $day => $date) {
  $sql = "SELECT matchid FROM matches WHERE cluster IN (".implode(",", $clusters).") AND start_date >= ".$date['timestamp']." AND start_date < ".$date['timestamp']."+86401;";
  ...
  for ($row = $query_res->fetch_row(); $row != null; $row = $query_res->fetch_row()) {
    $result["regions_data"][$region]['overview']["days"][$day]['matches'][] = $row[0];
  }
