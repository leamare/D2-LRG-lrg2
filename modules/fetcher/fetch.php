...
function fetch($match) {
  ...
  // TODO: remove Kez workaround
  if (in_array(145, array_column($t_matchlines, 'heroid'))) {
    $kez_items = array_filter($t_items, function($el) {
      return $el['hero_id'] == 145 && $el['category_id'] < 2 && !in_array($el['item_id'], [609, 1, 37, 48]);
    });
    if (!count($kez_items)) {
      $meta['items'];
      $meta['item_categories'];

      echo "..Missing Kez items, requesting OD...";

      $t_items = array_filter($t_items, function($el) {
        return $el['hero_id'] !== 145 || $el['category_id'] > 5;
      });
  
      if (empty($matchdata_od)) {
        $matchdata_od = $opendota->match($match);
      }
      foreach ($matchdata_od['players'] as $pl) {
        if ($pl['hero_id'] != 145) continue;
        if (empty($pl['purchase_log'])) continue;

        $j = array_search(145, array_column($t_matchlines, 'heroid'));

        $r_base = [
          'matchid' => $match,
          'playerid' => $t_matchlines[$j]['playerid'],
          'hero_id' => 145,
        ];
        $travel_boots_state = 0;

        foreach ($pl['purchase_log'] as $e) {
          if (($matchdata['duration'] ?? $matchdata['matches']['duration']) - $e['time'] < 60 || empty($e['time'])) continue;
  ...
  if(!empty($t_skill_builds) && ($schema['skill_builds'] ?? false) && ($lg_settings['main']['skill_builds'] ?? false)) {
    ...
    foreach ($t_skill_builds as $t) {
      ...
      $sql .= "\n\t({$t['matchid']}, {$t['playerid']}, {$t['hero_id']}, ".
        ...
        ? ",".(($t['attributes'] ?? false) ? "'".$t['attributes']."'" : "'[]'").",".
          "".($t['ultimate'] ?? 'null').""
        : "").
      "),";
    }
